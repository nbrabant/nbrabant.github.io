## docker
- [01 - installation](docker/01-installation.md)
- [02 - basic docker commands](02-basic-docker-commands.md)
- [03 - container ip address](03-container-ip-address.md)
- [04 - remove unused](04-remove-unused.md)
- [05 - attach detach](05-attach-detach.md)
- [06 - pull docker image](06-pull-docker-image.md)
- [07 - docker compose](07-compose.md)

## git
- [bitbucket ssh](bitbucket_ssh.md)
- [git commands](git_commands.md)

## linux
- [connexion par private key](connexion_par_private_key.md)
- [cross compiling](cross_compiling.md)
- [mise en place certificat ssl](mise_en_place_certificat_ssl.md)
- [post installation](post_installation.md)
- [securite](securite.md)
- [usefull commands](usefull_commands.md)
- [user and groups](user_and_groups.md)

## php
- laravel
    -  [performant](performant.md)
- prestashop
    - [aide memoire](aide_memoire.md)
    - [breadcrumb](breadcrumb.md)
- [array map et array walk](array_map_et_array_walk.md)

## vagrant
- [01 - installation](01-installation.md)
- [02 - basics](02-basics.md)
- [03 - with docker](03-with_docker.md)
